...
float get_hit_cylinder(t_vec3 O, t_vec3 D, t_cylinder cylinder)
{
	...
	t_vec3 D_perp;
	t_vec3 D_parallel;
	D_parallel = vec3_mul(cylinder.axis, vec3_dot(D, cylinder.axis));
	// this is the parallel component of D onto A
	D_perp = vec3_sub(D, D_parallel);
	...
	t_vec3 OC_parallel;
	t_vec3 OC_perp;
	OC = vec3_sub(O, cylinder.pos);
	OC_parallel = vec3_mul(cylinder.axis, vec3_dot(OC, cylinder.axis));
	OC_perp = vec3_sub(OC, OC_parallel);
	...
	float t = quadratic_roots(
		...
		2 * vec3_dot(D_perp, OC_perp),
		vec3_dot(OC_perp, OC_perp) - cylinder.radius * cylinder.radius);
	...
	float y = vec3_dot(vec3_sub(P, cylinder.pos), cylinder.axis);
	...
	// point Bottom center:
	// 	Cb = C - A * (h/2)
	t_vec3 Cb = vec3_sub(cylinder.pos, vec3_mul(cylinder.axis, cylinder.height / 2));
	// point Top center:
	// Ct = C + A * (h/2)
	t_vec3 Ct = vec3_add(cylinder.pos, vec3_mul(cylinder.axis, cylinder.height / 2));

	float t1 = get_hit_disk(O, D, Cb, cylinder.axis, cylinder.radius);
	float t2 = get_hit_disk(O, D, Ct, cylinder.axis, cylinder.radius);

	t1 = min_positive_f(t1, t2);
