...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/09 12:17:21 by wiwu          #+#    #+#                 */
/*   Updated: 2026/03/10 10:54:24 by wiwu         ###   ########.fr           */
/*                                                                            */
/* ************************************************************************** */
...
static const t_instruction g_plane_instruction
    = { "pl", OBJ_PLANE, false, offsetof(t_object, data),
          {
...
static const t_instruction g_cone_instruction
    = { "cn", OBJ_CONE, false, offsetof(t_object, data),
          {
...
static bool do_instruction(char **tokens, t_app *app, t_instruction instruction)
{
    t_parse_args args;
...
static bool check_uniqueness(char *token, const t_instruction *instruction)
{
    ...
    while (++i < INSTRUCTION_COUNT) {
        if (instruction->type == g_all_instructions[i]->type) {
            if (unique[i])
                return (
                    print_err("error, duplicated field: @\n", token, false));
            unique[i] = true;
            break;
        }
    }
